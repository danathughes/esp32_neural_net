...
void send_data(void *pvParameters)
{
    ...
    while (1) {
        ...
        if(tx_terrain_ready)
        {
//            ESP_LOGI(TAG, "sending terrain...");
//            printf("Terrain ID: %d\n", (int) terrain_class);
            int buff_size = NUM_TERRAINS * sizeof(float);
            int to_write = NUM_TERRAINS * sizeof(float);       

            //send function
            while (to_write > 0) {
                len = send(connect_socket, terrain_buffer + (buff_size - to_write), to_write, 0);
